...
NOTE ON A FASTER ALTERNATIVE: you generally do NOT need this script.
Hugging Face mirrors the same official CC0 Lichess puzzle data behind a
small, paginated JSON API (no multi-hundred-MB download required):

    https://datasets-server.huggingface.co/rows?dataset=Lichess/chess-puzzles&config=default&split=train&offset=0&length=100

Each call returns up to 100 puzzle rows as plain JSON (a few dozen KB).
That's what was used to seed the bundled puzzles.json shipped with this
project. Use THIS script instead only if you specifically want to filter
by exact rating range / theme across the full 6M+ puzzle set, or want
more than a few thousand puzzles at once — the HF endpoint above is
simpler for "just give me a few hundred more puzzles."

WHY THIS SCRIPT IS SEPARATE, NOT BUNDLED INTO THE APP: the official
database is a single ~250MB+ compressed CSV covering several million
puzzles. Downloading, filtering, and shipping that at runtime would be
enormous overkill for a chess app — instead, you run this script ONCE (or
occasionally, to refresh) on your own machine, and it produces a small,
pre-filtered puzzles.json that the app loads instantly.
